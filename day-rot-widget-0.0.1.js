...
if (config.runsInApp) {
    ...
    const options = ["Run Scipt", "Pick Background Image", "Reset Background Color"];
    const selectedIndex = await presentOptions(options);

    // Handle selected option
    if (selectedIndex === 1) {
        // Code to pick background image from photo gallery
        const backgroundImage = await Photos.fromLibrary();
